...
def on_tick(now,ft,poses):
	(pose_lh,vel_lh)=getpose(poses,state.controllers[1])
	(pose_rh,vel_rh)=getpose(poses,state.controllers[0])
...
#	print("___ WTF ",-pose_rh[3])
	...
	if pose_hmd[2]>pose_rh[2] or pose_hmd[2]<pose_lh[2] or d_r<0.3:# or math.dist(pose_rh,pose_hmd)>math.dist(pose_lh,pose_hmd):
		godown(ft,False)
	else:
		godown(ft,True)
