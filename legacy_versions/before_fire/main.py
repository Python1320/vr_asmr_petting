...
oscClient = SimpleUDPClient('127.0.0.1', 9000)
...
trackings = [
	...
	# TrackConfig(name="head", osc_message="/avatar/parameters/petting_volume")
]
...
def loop():
	global prev_ts
	poses = vr_system_fut.getDeviceToAbsoluteTrackingPose(openvr.TrackingUniverseStanding, 0, trackings)
	now = time.time()
...
def run(controllers):
	for t in trackings:
		if t.name == 'left':
			...
			t.name = t.name or t.serial
			t.device_id = getDeviceIDbySerial(t.serial or t.name, vr_system_fut)
...
def main():
	controllers = wait_controllers(vr_system_fut)
	if not controllers:
		return
	plugin_zbye.init(vr_system_fut, controllers)

	dump_devices(vr_system_fut)
	run(controllers)
